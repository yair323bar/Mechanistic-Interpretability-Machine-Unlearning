...
Forgetting: How much the intervention reduces accuracy on WMDP-Bio.

Utility preservation: How much general performance is preserved on the Retain dataset.
...
Measure the activity of every model layer on WMDP-Bio.

Compare the layer activity on WMDP-Bio with the activity on the Retain dataset.

Select layers using the localization scores and apply a causal intervention.

Compare the results with random and low-scoring layer controls.
...
Highest WMDP Residual-Update L2 scores

WMDP-vs-Retain

[11, 13]

Highest selective scores
...
Lowest WMDP localization scores
...
As in the main experiment, a positive Delta Retain means that performance decreased after the intervention.
...
Therefore, the result should not be interpreted as robust removal of the target knowledge.
...
Per-example WMDP baseline predictions

retain_baseline_predictions.csv

Per-example Retain baseline predictions

localization_scores.csv

WMDP, Retain, and selective scores for every layer

intervention_results.csv

Results for every method and intervention strength

all_strengths_table.csv

Full intervention-strength comparison

final_results.csv

Fixed-strength comparison at alpha = 0.25

causal_validation_results.csv

Retain results separated by MMLU subject

robustness_results.csv

Alternative-prompt robustness results
...
Residual-Update L2 score across layers
...
WMDP degradation compared with Retain degradation

robustness_comparison.png

Original and alternative prompt comparison
